...
async function listenToEvents() {
  let contractInstance = new web3.eth.Contract(abi, contractAddr.value); //생성됨
  contractInstance.events.TokensSent().on("data", (event) => {
  	eventResult.innerHTML = JSON.stringify(event) + "<br />=====<br />" + eventResult.innerHTML;
  }); //컨트랙트에 변화가 생길 경우 보이게 됨
    contractInstance.getPastEvents("TokenSent",{fromBlock: 0}).then((event) => {
  	eventResult.innerHTML = JSON.stringify(event) + "<br />=====<br />" + eventResult.innerHTML;
  }); //컨트랙트의 첫번째 내용 출력
}